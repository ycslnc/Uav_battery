...
class Map:
    def __init__(self, map_data):
        # self.start_land_zone = (map_data[:, :, 2] == 255).astype(bool)
        # self.obstacles = (map_data[:, :, 1] == 255).astype(bool)
        # self.nfz = (map_data[:, :, 0] == 255).astype(bool)
        # self.batterys = (map_data[:, :, 0] == 50).astype(bool)
        self.start_land_zone = map_data[:, :, 2].astype(bool)
        self.nfz = map_data[:, :, 0].astype(bool)
        self.obstacles = map_data[:, :, 1].astype(bool)
        self.battery_zone = np.logical_not(np.logical_or(self.nfz, self.start_land_zone))
